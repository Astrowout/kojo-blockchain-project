...
const Roadmap: FunctionComponent<RoadmapProps> = ({
	className = "",
	id = "",
}) => {
	const items = useMemo(() => [
		...
		<section className={cn(className, "")}>
			<div className='max-w-screen-xl mx-auto flex flex-col items-center gap-x-24 gap-y-12 px-6 sm:px-10 py-10 sm:py-16 justify-center min-h-[60vh]'>
				...
				<div className="mt-14">
					...
					<p className="font-sans uppercase text-sm text-center mt-3 mb-8">to a greener planet</p>
					<p className="font-mono text-center text-xs leading-6">Kojo is an ungoing project as long as there are yet opportunities to be explored to sustain water consumption. Let's start building!</p>
				</div>
				...
				<div className="w-full">
					...
					{items.map((item) => 	<li className="w-full bg-background p-8 rounded-md mb-5">
							...
							<p className="font-sans uppercase text-xs font-bold mt-2 mb-4">{item.title}</p>
							<p className="font-mono text-xs leading-6">{item.paragraph}</p>
						</li>)}
					</ul>
				</div>
