...
const Roadmap: FunctionComponent = ({ className }: any) => {
	const items = useMemo(() => [
		{
		...
		<section className={cn(className, "bg-background")}>
			<div className='max-w-screen-xl mx-auto flex flex-col items-center gap-x-24 gap-y-12 px-6 sm:px-10 py-10 sm:py-16 justify-center min-h-[60vh]'>
				...
				<div className="mt-14">
					...
					<p className="font-title uppercase text-sm text-center mt-3 mb-8">built right from the beginning</p>
				...
				<div>
					...
					{items.map((item) => 	<li className="flex flex-col items-center mx-3 mb-6" >
							...
							<p className="font-title uppercase text-xs mt-2 mb-4 md:w-20 text-center">{item.title}</p>
						</li>)}
					</ul>
				</div>
